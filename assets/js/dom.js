...
function addBook (event) {
  ...
  books.push (book), document.dispatchEvent (newBook);
}
...
function findBook (event) {
  ...
  if (searchValue.value == '') {
    ...
    p.innerText =
      'Menampilkan hasil pencarian buku dengan judul "' +
      searchValue.value +
      '"';
...
function showRead (n) {
  const num = Number (n.target.id),
    idx = books.findIndex (function (event) {
      return event.id === num;
    });
  (books[idx] = {...books[idx], isComplete: true}), document.dispatchEvent (
    newBook
  );
}
...
function showUnread (n) {
  ...
  (books[idx] = {...books[idx], isComplete: false}), document.dispatchEvent (
    newBook
  );
}
...
function showBook (books) {
  ...
  for (const book of books) {
    ...
    if (
      (div_item.appendChild (h2), div_item.appendChild (
        p
      ), div_item.appendChild (p2), book.isComplete)
    ) {
      ...
      (unread_button.id = book.id), (unread_button.innerText =
        'Belum Selesai Dibaca'), unread_button.classList.add (
        'blue'
      ), unread_button.addEventListener ('click', showUnread);
      const del_button = document.createElement ('button');
      (del_button.id = book.id), (del_button.innerText =
        'Hapus Buku'), del_button.classList.add (
        'red'
      ), del_button.addEventListener (
        'click',
        deleteBook
      ), div_action.appendChild (unread_button), div_action.appendChild (
        del_button
      ), div_item.appendChild (div_action), readBook.appendChild (div_item);
    } else {
      const div_action = document.createElement ('div');
      div_action.classList.add ('action');
      const read_button = document.createElement ('button');
      (read_button.id = book.id), (read_button.innerText =
        'Selesai Dibaca'), read_button.classList.add (
        'green'
      ...
      (del_button.id = book.id), (del_button.innerText =
        ...
        'click',
        deleteBook
      ), div_action.appendChild (read_button), div_action.appendChild (
        del_button
      ), div_item.appendChild (div_action), unreadBook.appendChild (div_item);
    }
  }
}
